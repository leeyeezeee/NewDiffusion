...
class NodeMasking:
    ...
    def remove_empty_edges(self, graph):
        ...
        graph.edge_index = graph.edge_index[:, graph.edge_attr.view(-1) != self.EMPTY_EDGE]
        graph.edge_attr = graph.edge_attr[graph.edge_attr.view(-1) != self.EMPTY_EDGE]
    ...
    def remove_masked_edges(self, graph):
        ...
        graph.edge_index = graph.edge_index[:, graph.edge_attr.view(-1) != self.EDGE_MASK]
        graph.edge_attr = graph.edge_attr[graph.edge_attr.view(-1) != self.EDGE_MASK]
