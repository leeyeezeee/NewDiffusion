...
from mas.tools.reader.readers import JSONReader
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run MultiArith experiment.")
    ...
    parser.add_argument('--train_set_size', type=int, default=10, help="Size of the training set")
    ...
    parser.add_argument('--dataset', type=str, default="multiarith", help="Dataset name")
    parser.add_argument('--domain', type=str, default="gsm8k", help="Reuse GSM8K domain prompt set")
...
async def generate_initial_dataset(args):
    ...
    raw_dataset = JSONReader.parse_file('data/MultiArith/MultiArith.json')
    dataset = multiarith_data_process(raw_dataset)
    print(f"Loaded {len(dataset)} MultiArith dataset")
...
async def evaluate(
    ...
    dirpath = mas_ROOT / "cache/MultiArith/graphs"
    ...
    for i_batch in tqdm(range(num_batches), desc=f"Processing {current_mode}-{current_agent_num}"):
        ...
        for i, result in enumerate(results):
            ...
            predict_answer = multiarith_get_predict(raw_answer)
            ...
            total_solved += 1
            name = "_".join(map(str, ['multiarith', metadata['record_idx'], current_mode, current_agent_num, is_solved]))
...
async def train_graph_diffusion_model(args):
    # load cached dataset
    graph_dir = mas_ROOT / "cache/MultiArith/graphs"
    role_dir = mas_ROOT / "cache/MultiArith/roles"
    ...
    model_dir = mas_ROOT / "cache/MultiArith/models"
...
async def evaluate_graph_diffusion_model(gd_framework, args, test_dataset):
    ...
    if test_dataset is not None:
        ...
        dataset = JSONReader.parse_file('data/MultiArith/MultiArith.json')
    ...
    with open('multiarith_accuracy.txt', 'a+', encoding='utf-8') as f:
...
async def main():
    ...
    graphs_dir = mas_ROOT / "cache/MultiArith/graphs"
    ...
    models_dir = mas_ROOT / "cache/MultiArith/models"
