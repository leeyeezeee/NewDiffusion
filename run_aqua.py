...
from mas.prompt.aqua_prompt_set import ROLE_DESCRIPTION
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run AQUA experiment.")
    ...
    parser.add_argument('--domain', type=str, default="aqua", help="Domain name, same as dataset name")
...
async def generate_initial_dataset(args):
    ...
    raw_dataset = JSONLReader.parse_file('data/AQuA/AQuA.jsonl')
    dataset = aqua_data_process(raw_dataset)
    print(f"Loaded {len(dataset)} AQuA dataset")
...
async def evaluate(
    ...
    dirpath = mas_ROOT / "cache/AQuA/graphs"
    ...
    for i_batch in tqdm(range(num_batches), desc=f"Processing {current_mode}-{current_agent_num}"):
        ...
        for i, result in enumerate(results):
            ...
            total_solved += 1
            name = "_".join(map(str, ['aqua', metadata['record_idx'], current_mode, current_agent_num, is_solved]))
...
async def train_graph_diffusion_model(args):
    # load cached dataset
    graph_dir = mas_ROOT / "cache/AQuA/graphs"
    role_dir = mas_ROOT / "cache/AQuA/roles"
    ...
    model_dir = mas_ROOT / "cache/AQuA/models"
...
async def evaluate_graph_diffusion_model(gd_framework, args, test_dataset):
    ...
    if test_dataset is not None:
        ...
        dataset = JSONLReader.parse_file('data/AQuA/AQuA.jsonl')
    ...
    with open('aqua_accuracy.txt', 'a+', encoding='utf-8') as f:
...
async def main():
    ...
    graphs_dir = mas_ROOT / "cache/AQuA/graphs"
    ...
    models_dir = mas_ROOT / "cache/AQuA/models"
